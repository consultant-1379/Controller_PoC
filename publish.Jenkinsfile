...
pipeline {
    ...
    environment {
        RELEASE = "true"
        TEAM_NAME = "$DEVELOPER_NAME - Data Controller"
    ...
    stages {
        stage('Clean') {
            steps {
                echo 'Inject settings.xml into home .m2 directory:'
...
def bumpVersionPrefixPatch() {
    ...
    sh 'docker run --rm -v $PWD/VERSION_PREFIX:/app/VERSION -w /app armdocker.rnd.ericsson.se/proj-eric-oss-drop/utilities/bump patch'
